...
$font_color=imagecolorallocate($image, 0,0,200); /* rbg shadow color*/
$font_color2=imagecolorallocate($image, 0,200,0); /* rbg shadow color*/
...
for ($i=0; $i<$count; $i++)
{
	...
	imagettftext($image, $font_size-1, $char_angle+$char_angle_shadow*(rand(0, 1)*2-1), $start, $char_align, $font_color2, $font_file, $char);
	$start+=$interval;
	$str.=$char;
}
